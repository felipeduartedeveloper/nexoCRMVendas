  ...
  return (
    <div className="flex h-full flex-col">
      <header className="flex items-center justify-between border-b border-ink-200 px-6 py-4">
        <div>
          <h1 className="text-xl font-bold text-ink-900">Modelos de email</h1>
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        {!isLoading && data.length === 0 && (
          <div className="grid place-items-center p-16 text-center">
            <Mail className="h-12 w-12 text-ink-300" />
            <h2 className="mt-3 text-base font-semibold text-ink-900">Sem modelos ainda</h2>
        ...
        {!isLoading && data.length > 0 && (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {data.map((t) => (
              <div
                key={t.id}
                className="rounded-lg border border-ink-200 bg-white p-4 shadow-card transition-shadow hover:shadow-elevated"
              >
                <div className="flex items-start justify-between">
                  <div className="min-w-0">
                    <h3 className="truncate font-semibold text-ink-900">{t.name}</h3>
                    {t.category && (
                      <span className="mt-1 inline-block rounded-md bg-ink-100 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-ink-600">
                        {t.category}
                      </span>
                    )}
                  </div>
                </div>
                <p className="mt-2 text-sm text-ink-700">{t.subject}</p>
                <div
                  className="mt-3 line-clamp-3 max-h-16 overflow-hidden rounded-md bg-ink-50 p-2 text-xs text-ink-600"
...
function TemplateEditor({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4">
      <div className="flex h-[80vh] w-full max-w-4xl flex-col rounded-xl bg-white shadow-elevated">
          ...
          <div className="flex flex-col gap-3 overflow-y-auto">
            ...
            <label className="block flex-1">
              ...
              <textarea
                ...
                rows={16}
                className="h-full w-full rounded-md border border-ink-200 px-2 py-1.5 font-mono text-xs"
              />
            </label>
          </div>
          <div className="overflow-auto rounded-md border border-ink-200 bg-white p-3">
            <p className="mb-2 text-xs font-semibold uppercase text-ink-500">Preview</p>
            <div className="rounded border border-ink-100 p-3">
              <p className="mb-2 text-sm font-semibold text-ink-900">{subject || '(sem assunto)'}</p>
