  ...
  return (
    <div className="space-y-4">
      ...
      {!isLoading && data.length === 0 && (
        <div className="grid place-items-center rounded-lg border border-dashed border-ink-200 p-12 text-center">
          <WebhookIcon className="h-10 w-10 text-ink-300" />
            ...
            <tbody className="divide-y divide-ink-200 bg-white">
              {data.map((w) => (
                <tr key={w.id} className="hover:bg-ink-50">
                  ...
                  <td className="px-3 py-2.5">
                    <div className="font-medium text-ink-900">{w.name ?? '(sem nome)'}</div>
                    <code className="block max-w-xs truncate text-[11px] text-ink-500">
                      {w.targetUrl}
                    </code>
                  </td>
                  <td className="px-3 py-2.5">
                    <span
                      className="inline-block cursor-help rounded-md bg-ink-100 px-1.5 py-0.5 text-[11px] font-semibold text-ink-700"
                      title={w.events.join('\n')}
                    >
                  ...
                  <td className="px-3 py-2.5 text-ink-700">
                    {w.lastStatusCode ?? <span className="text-ink-400">—</span>}
                  </td>
                  <td className="px-3 py-2.5">
